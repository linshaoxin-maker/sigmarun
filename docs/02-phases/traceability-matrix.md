...
| FEAT-010（BDD-006-06/07 + BDD-008-01…05 · 14 §4 · 16 §4/§7 · 18 #7/8/20/36–38 · AUD-017 inline 化 · D7/D11/D15 · NFR-004 · 子项 010.1–.3） | packages/dispatch/src/verify.ts；packages/core/src/{integrate,export}.ts；deps_satisfied_when 策略位；cli 五路由 | dispatch/test/verify.test.ts（5）+ core/test/{integrate,export}.test.ts（8）—— 172/172 绿，RED 13 先行 | ✅ 交付（覆盖 89.7%/73.5%；**MVP 主链闭合，北极星全链真机走通**；deps_satisfied_when 字段名回填 10 §6 挂账；[FEAT-010/verification.md](../05-features/FEAT-010/verification.md)） |
| FEAT-011（UC-009 · BDD-009-01/02/05/06（-03/-04 读路径已随 005/006 锁定）· 25 全文 · D19 · INV-012 项目级 · 18 #47/48 · AUD-036…040） | packages/context/src/memory-promote.ts；audit +5 规则；watch +风险；doctor +1 检查；cli 两路由 | context/test/memory-promote.test.ts（8 用例）—— 180/180 绿 | ✅ 交付（覆盖 89.4%/73.2%；**P4 特性全集交付完毕**；RED 记录偏离一次登记；[FEAT-011/verification.md](../05-features/FEAT-011/verification.md)） |

| ROAD-P1/P2（roadmap 可观性+故障降级+发布纪律 · docs/22 演进） | events/doctor --fix/worktree prune/--verbose；migrate/backups+restore/release 自动化 | 各自单测 + CHANGELOG 记录（266→…） | ✅ 交付（2026-07-13/14；语料库回写随整改 R4 补齐） |
| FEAT-LW（轻量模式 · docs/26 · D21 · INV-007 修订） | run import --lightweight/done/claim 自注册/adapter v0.3–0.4 | core/test/lightweight.test.ts + 模式墙/终局/审计档用例 | ✅ 交付（宪法随整改 R1 落 docs/26） |
| REM-R0…R4（整改 · docs/02-phases/remediation-design-2026-07-15.md · D21–D24 · S1–S13） | 32+ 提交：P0/模式墙/终局/判据/block/reopen/--force/TxKernel/needs_user/agent 视图/人面/watch/guidance/withRunTx 归一/state-machine 下沉/fsync/对账测试 | 311/311（+45 vs 审查基线 266）；13 场景回归 + 4 张机器对账 + 2 真机冒烟脚本 | ✅ 交付（P-1…P-4 验收命题成立；发布解锁待用户侧） |
