...
**import 防重（D17）**：gateway 对 payload 规范化后计算内容哈希，存入 `run.json.source.payload_hash`；再次 import 相同哈希的 payload → 拒绝（reason code `duplicate_payload`，exit 6，FEAT-002 实现期 backflow 定名）并指向既有 RUN，`--force` 可显式创建重复 run。防的是同一份计划被误跑两次造成的交叉运行。
