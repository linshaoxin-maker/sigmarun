...
| **D21** | **轻量模式入宪 + report 终局**（2026-07-15 产品负责人裁决，全面审查 S3/S8/S10） | 轻量模式获得宪法地位（[26](26-lightweight-mode.md)）：INV-007 修订为"full 模式永不放开、轻量显式豁免且留痕"；模式墙（submit/review/verify/integrate 在轻量 run 返回 mode_mismatch）；终局复用既有链 active→(全 done 后显式 report)→reported→archived，不加新状态；audit 轻量档（AUD-011/016/017/019 降 info） | 26 全文；15 §9 修订注；18 §4 修订注 |
| **D22** | **INV-008 owner 判据改实质贡献**（2026-07-15 产品负责人裁决，全面审查 S1） | 评审/验证独立性的排除集从"曾持有 claim"改为"提交过 evidence 的人 ∪ 当前持有者"——旧判据与租约接管相乘将评审门永久焊死并诱导第三身份洗白；残余风险（接管者提交含前任未提交代码，前任评审自己旧行）如实记档，缓解=评审永久留痕+AUD-015 同判据复查；historicalOwners 仅保留为 unblock 的所有权判据 | 18 §4.C AUD-015 修订注 |
| **D23** | **audit 包定位=规则+重放+修复**（2026-07-15 产品负责人裁决） | 修订 docs/20 §4.6 的"audit 只读"契约：repair 与检测共用 foldLedger 单真值源，留在 audit 包；约束改为"audit 的唯一写路径是 repair，且必须经 TxKernel+备份"（R3 落地 TxKernel 全量迁移时机械强制） | 20 §4.6（R4 回写） |
| **D24** | **语料库宪法权威 + 机器对账**（2026-07-15 产品负责人裁决，漂移根因裁断） | 语料库仍是唯一权威；恢复"每特性带回写"纪律；三张对账测试（命令表/事件目录/状态机表）常驻 CI 使漂移=红灯（R4 落地）；本轮 reconciliation 见 remediation-design §4 R4 清单 | docs/02-phases/remediation-design-2026-07-15.md §5–6 |
